...
def test_get_clcov_from_data_matrix():
    ...
    c["cov"]["cls_from_data"] = {"DESgc-DESgc": {}, "default": "all"}
    d = Data(data=c)
    mat = {("DESgc", "DESgc"): "all"}
    assert mat == d._get_clcov_from_data_matrix(return_default=False)
    assert (mat, "all") == d._get_clcov_from_data_matrix()
    ...
    d = Data(data=c)
    mat = {
        ("DESgc__0", "DESgc__0"): "all",
        ("DESgc__1", "DESwl__1"): "all",
...
def test_get_requested_survey_cls_matrix():
    ...
    assert (mat, "None") == d._get_requested_survey_cls_matrix(
        return_default=True
    )
    ...
    assert (mat, "None") == d2._get_requested_survey_cls_matrix(
        return_default=True
    )
    ...
    assert d2.data["cls"] == data["cls"]
    assert mat == d2._get_requested_survey_cls_matrix(return_default=False)
    assert (mat, "all") == d2._get_requested_survey_cls_matrix(
        return_default=True
    )
...
def test_initizalization():
    input_file = get_input_file()
    config = read_yaml_file(input_file)
    # Check that inputting both a file path and config dictionary raises an
    # error
    with pytest.raises(ValueError):
    ...
    # Check error is raised when override and ignore_existing_yml are True
    with pytest.raises(ValueError):
...
def test_get_tracers_used():
    ...
    assert sorted(data.get_tracers_used(True)) == sorted(
        ["DESgc__0", "DESwl__0", "DESwl__1", "DESwl__2", "DESwl__3", "PLAcv"]
    )
    remove_yml_file(data.data)
