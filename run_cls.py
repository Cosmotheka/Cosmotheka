...
def get_pyexec(
    comment, nc, queue, mem, onlogin, outdir, batches=False, logfname=None
):
    if batches:
        pyexec = ""
    else:
    ...
    if not onlogin:
        ...
        pyexec = "addqueue -o {} -c {} -n 1x{} -s -q {} -m {} {}".format(
            logfname, comment, nc, queue, mem, pyexec
        )
...
def launch_cov_batches(
    ...
    for ni, (cw, trs_list) in enumerate(cwsp.items()):
        ...
        for trs in trs_list:
            ...
            if (os.path.isfile(fname) and (not recompute)) or check_skip(
                data, skip, trs
            ):
                continue
    ...
    for nodei in range(nnodes):
        ...
        with open(sh_name, "w") as f:
            f.write("#!/bin/bash\n\n")
            for i, shi in enumerate(
                sh_tbc[nodei * njobs : (nodei + 1) * njobs], 1
            ):
...
def launch_cls(
    data, queue, njobs, nc, mem, fiducial=False, onlogin=False, skip=[]
):
    #######
    #
    ...
    for tr1, tr2 in cl_tracers:
        ...
        if not fiducial:
            ...
            pyrun = "-m cosmotheka.cls.cl {} {} {}".format(
                args.INPUT, tr1, tr2
            )
        else:
            pyexec = get_pyexec(comment, nc, queue, 2, onlogin, outdir)
            pyrun = "-m cosmotheka.cls.cl {} {} {} --fiducial".format(
                args.INPUT, tr1, tr2
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "compute", type=str, help="Compute: cls, cov or to_sacc."
    )
    parser.add_argument(
        "-n", "--nc", type=int, default=28, help="Number of cores to use"
    )
    parser.add_argument(
        "-m",
        "--mem",
        type=int,
        default=7.0,
        help="Memory (in GB) per core to use",
    )
    parser.add_argument(
        "-q", "--queue", type=str, default="berg", help="SLURM queue to use"
    )
    parser.add_argument(
        "-N",
        "--nnodes",
        type=int,
        default=1,
        help="Number of nodes to use. If given, the jobs will be launched all in the same node",
    )
    parser.add_argument(
        "-j",
        "--njobs",
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Run the jobs in the login screen instead appending them to the queue",
    )
    parser.add_argument(
        "--skip",
        default=[],
        nargs="+",
        help="Skip the following tracers. It can be given as DELS__0 to skip only DELS__0 tracer or DELS to skip all DELS tracers",
    )
    parser.add_argument(
        "--override_yaml",
        default=False,
        action="store_true",
        help="Override the YAML file if already stored. Be ware that this could cause compatibility problems in your data!",
    )
    parser.add_argument(
        "--batches",
        default=False,
    ...
    if args.clean_lock:
        ...
        if args.batches:
            ...
            launch_cov(
                data, queue, njobs, args.nc, args.mem, onlogin, args.skip
            )
        ...
        if args.to_sacc_use_nl and args.to_sacc_use_fiducial:
            ...
            use = "cls"
        launch_to_sacc(
            data, args.to_sacc_name, use, queue, args.nc, args.mem, onlogin
        )
    else:
        raise ValueError(
