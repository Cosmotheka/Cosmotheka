...
def get_config(mask_ps=False):
    c = {'exposure_map': 'xcell/tests/data/exp_rosat.fits',
         ...
         'point_source_catalog': 'xcell/tests/data/cat_ps_rosat.fits',
...
def make_rosat_data(e0=0., ef=4., expval=200.):
    ...
    cat = Table({'RA_DEG': ra[:10],
                 'DEC_DEG': dec[:10],
                 'RATE': np.ones(10)})
    cat.write('xcell/tests/data/cat_ps_rosat.fits', overwrite=True)
...
def test_map_sanity_masked():
    ...
    assert np.all(mp == hp.read_map(fn))
    os.remove(fn)
    os.remove('xcell/tests/data/mask_mask_ROSAT_coordC_ns32.fits.gz')
    clean_rosat_data()
